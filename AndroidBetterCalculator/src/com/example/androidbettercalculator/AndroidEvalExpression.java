...
public class AndroidEvalExpression {
      ...
      for (String token: tokens) {
        ...
        if (token.length() == 0) // Blank space
          ...
          while (!operatorStack.isEmpty() &&
                   higherPrec(operatorStack.peek(),token.charAt(0)))
   ...
   *  apply it the operands on the operandStack */
  ...
  /** Determines if the first operator has higher precedence than the
   *  second operator or the same precedence but on left, so higher */
